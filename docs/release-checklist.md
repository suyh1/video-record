...
- [x] 前端 lint、typecheck、38 个测试文件中的 227 项组件测试、OpenAPI 生成检查和 build 通过。
- [x] 27 项专项 Playwright E2E（1 项初始化、26 项 UI/代理回归）通过，WCAG 2.2 AA 阻断违规为 0。
- [x] 性能与四种中断恢复门禁通过。

### 娱乐化 UI 回归补充

- [x] TMDB 元数据与图片均通过服务端代理；登录页、首页、影库和详情页观察到的图片请求均使用同源签名 URL。
- [x] 浏览器未直接请求 `api.themoviedb.org`、`image.tmdb.org` 或其子域名。
- [x] 登录页与首页在全部 TMDB 图片失败时，亮暗主题均回落为纯白背景且不加载内置精选图。
- [x] 登录、首页、影库和详情页完成 `375x812`、`768x1024`、`1440x900` 三视口与亮暗双主题视觉回归。
- [x] WCAG 2.2 AA、键盘操作、200% 缩放和 reduced-motion 回归通过；日历日期可由键盘切换到获得焦点的日程视图。
...
- 前端：`npm ci`/lint/typecheck/API check/build/audit 通过；38 个测试文件、227 项测试通过，npm 漏洞 0，主应用 chunk 413.22 kB（gzip 123.76 kB）。
- E2E：27/27 通过（1 项初始化、26 项 UI/代理回归）；axe 阻断违规 0，键盘、200% 缩放、reduced-motion 和三视口双主题回归通过；48 张视觉快照使用零像素差异容差。
...
- 安全与供应链：actionlint、workflow policy、release/manifest/image scan 脚本、shell syntax、工作树与完整 Git 历史 gitleaks 均通过。
