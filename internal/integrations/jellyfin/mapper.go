...
func mapHistoryEvent(row activityRow, item itemResponse) (integrations.HistoryEvent, error) {
	...
	if err != nil || row.ItemID != item.ID || item.RunTimeTicks < 0 {
	...
	return integrations.HistoryEvent{
		ID: "jellyfin:" + row.RowID, PlayedAt: playedAt,
...
func parsePlayedAt(value string, historyDate time.Time) (time.Time, error) {
	...
	if !historyDate.IsZero() {
		parsed, err := time.ParseInLocation("3:04 PM", value, time.UTC)
		if err == nil {
			year, month, day := historyDate.UTC().Date()
			return time.Date(year, month, day, parsed.Hour(), parsed.Minute(), 0, 0, time.UTC), nil
		}
	}
