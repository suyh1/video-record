...
func TestHouseholdMembersPrivacySharingAndSharedEvents(t *testing.T) {
	...
	require.False(t, sharing.ShareRating)
	require.False(t, sharing.ShareReview)
	require.Nil(t, sharing.SharedReview)
	_, err = service.Sharing(ctx, admin.ID, member.ID, movie.ID)
	require.ErrorIs(t, err, ErrForbidden)
	...
	sharing, err = service.Sharing(ctx, member.ID, member.ID, movie.ID)
	require.NoError(t, err)
	require.Equal(t, shared, sharing)
