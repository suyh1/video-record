...
打开 `http://localhost:8080` 完成首位管理员初始化。Compose 部署只使用 `docker-compose.yml`；默认值适合本机或可信局域网的纯 HTTP 验证。经 HTTPS 反向代理部署，或需要 TMDB、媒体服务器集成时，先按[部署文档](docs/deployment.md)直接修改 Compose 中的对应值。
...
Vite 开发服务器代理 `/api` 到 Go 服务。直接运行后端时继续通过进程环境传入配置。Compose 中填写真实凭据后，不要提交对应的本机修改，并把加密密钥另存到安全密钥存储中。
