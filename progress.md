...
- 已核对 `main` 分支与干净工作树，起点为 `534e226`。
- 已查看用户两张截图并沿前后端数据流定位：当前季标题来自实时 TMDB 目录，归档详情读取本地稀疏单集空名称，最终由前端回退为“未命名”。
...
- Task 13 已把 E2E 种子升级为 version 2，并新增电影秒级完成/再刷、单集圆圈/时间编辑/未来值、第二季记录隔离与归档详情旅程；电影旅程已通过。
- 真实 E2E 首轮两个剧集失败已完成证据追踪：季面板通用 `grid-area` 产生隐式列并把标题压为 0px；后端把全剧累计集数错误与本季总集数比较，导致第二季整季请求返回 400。
- 已先增加第二季 `totalEpisodes: 2`、累计集数 `4/5` 的真实 HTTP 回归测试，准备确认 RED 后实施最小领域校验修正。
- 第二季累计集数 HTTP 测试已取得预期 400 RED；领域校验只保留累计集数为正的约束后，同一测试转绿。
- 季面板以组合选择器清除通用 `grid-area` 后，真实 E2E 已越过第二季标题可见断言；整季写入也成功进入完成态。当前只收紧两处 Playwright 严格模式定位器后重跑。
- 收紧定位器后单集时间旅程通过；季归档旅程暴露真实状态回退：整季完成返回 `completed`，随后私人记录请求却发送旧的 `watching`。已增加同轮次投影更新的组件回归测试，准备确认 RED。
- 季级 payload 组件测试取得 `watching` RED 并在使用最新 prop 状态后转绿；真实旅程随后定位到服务端仍要求已完成轮次的私人字段更新重复提交 `watchedAt`。已增加保留既有完成时间的 HTTP 回归测试。
- 已完成完成时间保留规则的 HTTP RED/GREEN；定向真实旅程 4/4、归档弹窗 axe/键盘子集 5/5、完整无障碍/三视口结构 5/5 通过。
- 六张详情明暗基线已在结构门禁通过后强制重录；375x812 明暗图已人工确认单集两行布局、季级记录顺序、静态保存栏和多刷空态无截断。
- 768x1024 明暗图已人工确认平板单栏断点、累计集数/标题/时间列、个人记录面板与多刷空态均无溢出或遮挡。
- 1440x900 明暗图已人工确认分集主列与 400px 私人记录双栏正常，标题轨道不再为 0、时间列稳定，多刷区域保持全宽；六张详情基线人工复核完成。
- 旧 `PUT /records/{mediaID}` 已用 500 RED/405 GREEN 固定为移除；扁平事件新增/删除、旧媒体状态写和旧无季号进度仓储均已删除，合法写测试迁到当前轮次/再刷接口。
- OpenAPI 已移除 `updateRecord`/`UpdateRecordRequest` 并重新生成前端类型；幂等、CSRF、ETag、用户隔离和合同测试已迁到轮次模型。
- version 1 导入导出与同步中的死 helper 已清除；性能种子迁到 profile + 当前轮次 + 带 round_id 事件，精确种子/回滚测试及非 race 全仓测试通过。
- 完整 E2E 首轮 14/16 通过、1 失败、recovery 未运行；唯一失败是 accessibility 已归档第一季后目录身份仍存在，旧断言错误要求 `episodes=[]`。已改为验证当前轮所有集未看。
- 完整 E2E 第二轮 13/16 通过、2 失败、recovery 未运行；首个失败仍把标记一集后的季目录误断言为只有一项，导致撤销未执行并污染后续串行季隔离旅程。
- 已把断言收窄到 `watched=true` 子集必须且仅包含 S01E01；保留完整目录身份是新季级进度契约，不修改生产逻辑。
- 修正后定向旅程 4/4、完整 Playwright 16/16 通过；setup、六张详情视觉基线、WCAG/键盘/缩放、两条剧集时间旅程、季隔离归档、电影再刷及真实恢复均完成。
- 已进入最终门禁：按 CI 同源命令重跑前端静态/单元/API/构建/审计、7 个关键 Go 包精确覆盖率，以及工作树与 Git 历史凭据扫描。
- 前端 lint/typecheck、28 文件 73 项 Vitest、OpenAPI 漂移、production build 与 npm audit 已通过；覆盖率门禁在 records 73.1504% 失败，工作树 gitleaks 命中 3 项，Git 69 个提交历史无命中，正在做根因定位。
- 函数级 coverprofile 已定位 `sync_round.go` 为 0% 主缺口；gitleaks 脱敏报告已把命中限定在两个本地文件，下一步核对 Git/生成归属并补 records 包内行为覆盖。
- 两个凭据命中文件均为 Git/Docker ignored，本地凭据保持原样；已确定用 Git 控制文件隔离副本复验提交内容。同步轮次将补 records 包内真实事务测试以恢复 85% 门禁。
- 覆盖率精确缺口为 199 条，sync_round 文件总计 101 条；同步测试完成后将以新 profile 定向补足 version 2 导入/导出等真实分支。
- 新增 3 组 records 同步轮次事务测试并通过：非法作用域、电影部分/完成/自动分轮/来源冲突、剧集逐集/跨季/下一刷。sync_round 覆盖 84/101，records 升至 78.1623%，距 85% 还差 115 条。
- 新增 rich v2 导入/导出、CSV/校验矩阵和电影事件同步测试；定向运行发现显式空共同观看者被错误当作 no-op，已定位前后端双层根因，正在按回归测试修复。
- 当前轮次读模型已增加 `participantIds`：SQLite 回填已有电影共同观看者，HTTP/OpenAPI 返回稳定数组，表单回显并可显式清空；Go 定向测试、表单 9/9、typecheck 与 API 漂移检查通过。
- records 全包首次复验暴露新建轮次参与者 nil、重读空数组的不一致；失败运行覆盖约 82.7%。已从 `emptyRound` 统一为空切片，准备重新测量。
- records 全包转绿，精确覆盖 1410/1704（82.7465%），距 85% 还差 39 条；rich CSV/JSON 校验已分别达到 95.4%/100%，下一步补版本化标签、作用域和导入冲突分支。
- 版本化标签成功更新、指针 helper、空作用域、rich 幂等导入和跨用户轮次 ID 冲突测试通过；移除无引用 helper 后 records 为 1420/1701（83.4803%），还差 26 条。
- SQLite 关闭后的 13 个服务/仓储错误传播及再刷缺版本、详情缺 ID、观看事件缺身份测试通过；精确 records 覆盖升至 84.4797%，仍未达到 85%，继续补约 9 条。
- rich CSV 的自定义简介、归档时间和无轮次占位行补测后，7 包覆盖率门禁通过：records 85.0088%、media 86.4%、stats 85.1485%、household 86.6667%、storage 85.4195%、integrations 88.0%、sync 86.1472%。
- Git 跟踪文件与待提交新文件的隔离副本经 gitleaks 扫描约 2 MB、零命中；两个 ignored 本地文件保持原样且不参与提交内容门禁。
- 修改后 Go 常规全仓/vet、前端 lint/typecheck、28 文件 74 项测试、OpenAPI/build/audit 通过；首次全仓 race 仅 `TestSchedulerContinuesPollingAfterProviderFailures` 在 3 秒观察窗超时，未报告数据竞争，正在定向复现。
- 调度器故障恢复测试定向 race 连续 20 次全部通过（总计 5.6 秒），未稳定复现；保持代码与阈值不变，按原全仓 race 命令重新验证。
- 第二次 `go test ./... -race -count=1` 全仓通过，sync 包 9.1 秒完成且无数据竞争；保留现有调度器实现和 3 秒单条件观察窗。
- participantIds 读模型改造后的最终 Playwright 16/16 通过，视觉、WCAG/键盘/缩放、两条剧集时间旅程、季隔离归档、电影再刷、同步与恢复均无回归。
- 提交前 `git diff --check` 通过；正在对 53 个变更文件做本地代码审查，重点核对旧 v1 入口残留、消费者迁移和当前电影参与者隐私边界。
- 本地代码审查完成：旧写/事件路径仅余 405 与“不请求”回归，旧表仅余历史迁移测试；导入导出/同步/种子均使用轮次模型，参与者读取未越过用户作用域。trace viewer 已停止，19323 无监听。
- 应用内浏览器实时验收完成：桌面 `1440x900` 验证电影秒级 `step/max/value`、保存、再刷归档与详情；移动 `375x812` 页面 `scrollWidth=375`、无越界元素，归档弹窗边界 `16..359px` 且未被底栏遮挡，控制台 warning/error 为 0。
- Task 13 及观看轮次改造阶段 5/6 全部完成；本地后端 `18081` 与 Vite `http://127.0.0.1:5174/` 保持运行，使用独立临时数据库。
- 娱乐化 UI 重设计已完成设计文档、13 项实施计划及 Task 1-7；Task 8 初版提交为 `4a9335a`，针对首轮质量审查的修补提交为 `35d099d`。
...
## 2026-07-16 详情图片并发缺失修复

- 已恢复 planning-with-files 上下文并读取 brainstorming、systematic-debugging、TDD 与应用内浏览器约束；本轮先调查和确认设计，再实施。
- 已核对两张截图：同一影视每次刷新只有不同子集的海报/演员图成功，backdrop 正常；代码搜索定位到公开图片代理的固定并发槽位及饱和响应契约，尚未修改生产代码。
- 已完成代理数据流追踪：4 个 channel 槽位满后 handler 非阻塞返回 429，浏览器图片错误态不重试；现象、OpenAPI 和既有并发测试三方一致。下一步只读核对运行端口的实际网络响应，再提出修复方案。
- 旧并发测试 `-count=10` 通过并稳定复现第二请求 429、释放后第三请求 200；阶段 2 完成，进入方案确认。
- 用户已确认推荐方案；设计文档写入 `docs/plans/2026-07-16-tmdb-image-proxy-queue-design.md`，约束活动并发 4、总在途 32、取消释放和真正饱和时 429。
- writing-plans 产出 `docs/plans/2026-07-16-tmdb-image-proxy-queue.md`：先用延迟图片上游取得浏览器 RED，再改 Go 队列测试和实现，最后全仓与五次刷新验证。
- Task 1 浏览器 RED：目标测试 10.9 秒后收到 `{loaded:4,total:4}` 而非 5/5，其余 14 个依赖/代理测试通过。
- Task 2 Go RED：新增等待、admission 饱和和排队取消测试后，编译准确失败于缺失 `imageRequests` 字段；生产代码仍未修改。
- Task 3 GREEN：handler/router 增加 32 总在途、4 活动的双 semaphore；全部 `TestPublicTMDBImage` 通过。
- 慢图浏览器 GREEN：详情五张图片全部解码，五个签名代理响应均为 200；完整依赖链 15/15 通过。
- 提交前 review 补齐测试提前失败清理和 synthetic control 状态隔离；图片 handler race、lint、diff check 通过，修复提交为 `424d0fb`。

# 2026-07-16 详情图片并发缺失完成度核查

- 已读取适用的调试、TDD、文件计划和完成前验证流程。
- 已确认按用户要求直接在 `main` 工作；当前 HEAD 为 `424d0fb`，工作区只有跟踪文档改动。
- 正在进入根因与实现审计阶段，尚未宣称任务完成。
- 已完成提交实现与测试审计：根因修复边界和回归覆盖成立，进入新鲜复现与完整验证。
- Task 4 静态与单元门禁已完成：全仓 Go race/vet、前端 241 项测试、lint、build、API check 全部通过。
- 标准隔离 E2E 37/37 通过，慢图详情图片队列回归通过；准备启动新鲜本地环境完成应用内浏览器五次刷新。
- 应用内浏览器在 250ms 慢图环境完成 6 次刷新：每次 5/5 图片解码、5/5 使用签名代理、无失败占位，签名 URL 跨轮更新；浏览器 warning/error 为 0。
- 最后一轮后端图片请求 5/5 为 200、无 429；临时浏览器标签和合成 TMDB、后端、Vite 服务均已关闭。
- 原任务阶段 6 与本轮核查阶段全部完成，未发现需要继续修改的业务代码。

## 2026-07-16 登录与退出故障修复

- 已按用户要求以 `main` 为目标，恢复既有计划上下文。
- 已开始系统化排查；当前处于只读根因调查阶段，尚未修改业务代码。
- 首次跟踪文件补丁因跨文件尾部锚点不一致被整体拒绝；按真实末尾重放后已建立本轮阶段记录。
- 已审计前端认证客户端、AuthGate、账户设置、后端登录/退出、同源/CSRF 中间件、会话服务、现有测试与运行容器。
- 已确认本机 `18080` 容器存储就绪，但日志没有截图对应认证请求；需要用实际访问入口完成稳定复现后才能进入修复设计。
- 用户已批准“代理协议感知同源校验 + 同源幂等退出”的推荐方案；设计文档写入 `docs/plans/2026-07-16-auth-proxy-logout-design.md`。
- 设计文档提交为 `9c83add`；详细 TDD 实施计划写入 `docs/plans/2026-07-16-auth-proxy-logout.md`，本会话将在 `main` 顺序执行。
- Task 1 已取得预期 RED；正在实现只影响协议推断、不改 Host 信任边界的最小 GREEN。
- Task 1 GREEN 在 race 下通过并提交为 `c865a7a`。
- Task 2 已取得缺 CSRF、已撤销会话和无 cookie 三个预期 RED；正在实现仅同源的幂等退出。
- Task 2 GREEN 在 auth/httpapi race 选择集通过并提交为 `3f8df6d`。
- Task 3 已取得 OpenAPI 参数、OpenAPI 安全要求和前端空头三个预期 RED；正在同步客户端、契约与安全文档。
- Task 3 GREEN：4 个合约测试、客户端 9 项测试和 API 生成漂移通过，提交为 `6976707`。
- 新增真实浏览器回归并以 setup project 1/1 通过，提交为 `76f9d68`。
- 完整验证通过：Go race/vet、Vitest 242/242、lint、typecheck、production build、API check、Playwright 37/37。
- 应用内浏览器插件文件在长时间测试后从缓存消失，无法执行额外的应用内手动点击；仓库 Playwright 已完成同一真实 UI 旅程，当前源码开发服务的实时 HTTP 会话链也逐步验证通过。
- 本地代码安全审查无发现；认证修复五个阶段全部完成，开发地址 `http://127.0.0.1:15174/` 保持运行供用户试用。
- 收尾时已删除仓库内生成的 `.tmp`，以仓库外临时数据库重启 `28082` 后端；开发页和 ready 探针均返回 `200`，工作树只剩本轮跟踪文档。
