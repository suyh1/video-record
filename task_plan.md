...
- [x] 阶段 1：恢复上下文并核对截图、工作树与现有图片代理入口
- [x] 阶段 2：稳定复现同页多图并发失败并定位具体状态码/释放边界
- [x] 阶段 3：比较修复方案、确认失败语义与验收标准
- [x] 阶段 4：写入设计文档与详细实施计划
- [x] 阶段 5：按 TDD 实现根因修复
- [x] 阶段 6：完成全仓与连续刷新浏览器验证

### 本轮约束

- 继续直接在 `main` 修改，保留用户现有数据与前一轮签名同源图片代理边界。
- 不以无限前端重试掩盖服务端随机拒绝；必须先证明失败发生在哪一层，并保持图片尺寸、类型、体积和重定向安全限制。
- 验收需覆盖详情页海报、头图和全部有 `profilePath` 的演员图，多次刷新不得随机退回占位图。

### 本轮错误记录

- 根因调查未产生工具或代码错误；旧并发测试连续 10 次稳定证明槽位满时立即 429，当前生产代码尚未修改。
- 用户确认采用服务端有界等待队列：活动并发 4、总在途 32，取消即释放，真正饱和时保留 429。
- 详细 TDD 计划写入 `docs/plans/2026-07-16-tmdb-image-proxy-queue.md`；因用户明确要求直接在 `main` 且未要求子代理，本会话顺序执行，不创建 worktree/子代理。
- RED/GREEN 修复提交为 `424d0fb`：浏览器慢图从 4/5 转为 5/5，handler 等待/饱和/取消语义均有测试。

# 2026-07-16 详情图片并发缺失完成度核查

## 目标

确认 `main` 上详情图片并发缺失修复是否具备根因修复、回归测试和新鲜验证证据；若存在缺口，按 TDD 补齐并直接在 `main` 修复。

## 阶段

- [x] 阶段 1：确认分支、工作树和最近相关提交
- [x] 阶段 2：审计并发缺失根因、实现与测试覆盖
- [x] 阶段 3：复现现状并判断是否仍有缺口
- [x] 阶段 4：如有缺口，先取得失败回归再做最小修复（审计确认 `424d0fb` 已完成 RED/GREEN，无需新增业务改动）
- [x] 阶段 5：完成定向与全量验证，给出结论

## 错误

- 首次跟踪更新补丁包含多余 hunk 结束标记，第二次又使用了不唯一的章节上下文；两次均被 `apply_patch` 整体拒绝且无文件变化，改用当前尾部精确上下文更新。
- 应用内浏览器首次直达详情因初始化页的前端状态未恢复持久会话而显示登录页；在临时本地实例重新登录后详情正常，未计入刷新次数。
- 应用内浏览器 locator 批量读取两次触发 3 秒控制超时；页面快照与日志均正常，改用同一页面只读 DOM 检查逐轮验收，未重复失败调用。

## 2026-07-16 登录与退出故障修复

- [x] 阶段 1：确认 `main`、工作树、近期认证改动并稳定复现登录/退出故障
- [x] 阶段 2：追踪浏览器、前端客户端、Cookie/CSRF 与后端会话的数据流，确认根因
- [x] 阶段 3：比较最小修复方案并确认设计与测试边界
- [x] 阶段 4：按 TDD 补失败回归并实施根因修复
- [x] 阶段 5：完成定向、全仓和真实浏览器登录/退出验证

### 本轮错误记录

- 首次同时追加三个跟踪文件时使用了错误的跨文件尾部锚点，补丁整体拒绝且没有文件变化；已读取真实文件末尾后分别使用精确上下文。
- 对未跟踪实施计划使用 `rg` 检查尾随空格时，零匹配按工具语义返回 1，导致同命令后续状态检查未执行；文件没有匹配项，改用暂存后的 `git diff --cached --check` 作为正式门禁。
- 应用内浏览器的旧连接未跨过长时间测试保留，重新初始化时插件缓存中的 `browser-client.mjs` 已不存在；按浏览器规则停止该控制器，不切换到其他浏览器工具，改用已通过的仓库 Playwright 真实 Chromium 旅程和保留开发服务的实时 HTTP 链路完成验收。
- planning-with-files 的完成检查脚本无法识别本仓库长期累积计划中的中文阶段格式，输出 `0/0`；本轮五个阶段已在本节逐项勾选，并以测试、提交和实时状态交叉核对。
- 首次清理本轮生成的 `.tmp` 使用递归强制删除，被安全策略拒绝且目录未变；列出六个确切路径后逐个删除文件并用 `rmdir` 清空，开发数据库迁到仓库外。

### 实施与验证

- Task 1：代理协议感知同源校验，提交 `c865a7a`。
- Task 2：仅同源、无会话依赖的幂等退出，提交 `3f8df6d`。
- Task 3：前端、OpenAPI、生成类型与安全文档同步，提交 `6976707`。
- 浏览器回归：清标签页 CSRF、保留 HttpOnly cookie、退出并再次登录，提交 `76f9d68`。
- 全仓 Go race/vet、39 文件 242 项 Vitest、lint、typecheck、生产构建、API 漂移和 37 项 Playwright 全部通过。
- 当前源码开发环境保留在 `http://127.0.0.1:15174/`，使用独立的仓库外临时数据库。
